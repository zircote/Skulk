...
class Skulk_Client_Message
{
    ...
    {
        $priorities = array(self::PRIORITY_VERYLOW, self::PRIORITY_NORMAL,
        self::PRIORITY_MODERATE, self::PRIORITY_HIGH, self::PRIORITY_EMERGENCY);
        if (in_array($priority, $priorities)) {
            $this->priority = $priority;
        } else {
            require_once 'Skulk/Client/Exception.php';
            throw new Skulk_Client_Exception('invalid priority level submitted',
            501);
        }
        return $this;
    }
    ...
    {
        $return = array();
        foreach (array('url', 'priority', 'providerkey', 'apikey',
        'application', 'event', 'description') as $prop) {
            $m = 'get' . ucfirst($prop);
            $return[$prop] = $this->$m();
        }
        ;
